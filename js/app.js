...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (installBtn) {
    installBtn.addEventListener('click', async () => {
      if (!deferredPrompt) {
        toast('Install not ready yet — use the app a bit first.', 'skip');
        return;
      }
      ...
      if (outcome === 'accepted') {
        toast('App installed! Check your home screen.', 'done');
      } else {
        toast('Install canceled.', 'skip');
      }
    });
  }
});
...
function newImageID(prefix = 'img') {
